...
def ffprocess(acc,fflist,watermark,fontfile,scriptRepo,logfile):
	...
	with cd(acc): #cd into it
		...
		if len(concatlist) > 1:
			...
			ffconcatstring = 'ffmpeg -f concat -safe 0 -i concat.txt -c copy -ignore_unknown -map 0 rawconcat.mov'
			txtfile.close()			
			try:
		...
		try:
			flvstr = 'ffmpeg -i ' + mov + ' -i ' + watermark + ' -filter_complex "scale=320:180,overlay=0:0;[0:a:0][0:a:1]amerge=inputs=2[a]" -c:v libx264 -preset fast -b:v 700k -r 29.97 -pix_fmt yuv420p -c:a aac -ac 2 -map 0:v -map "[a]" -map -0:d? -timecode ' + segment[-2:] + ':00:00:00 -threads 0 ' + flv
		...
		try:
			mpegstr = 'ffmpeg -i ' + mov + ' -target ntsc-dvd -filter_complex "[0:a:0][0:a:1]amerge=inputs=2[a]" -map 0:v -map "[a]" -map -0:d -ac 2 -b:v 5000k -vtag xvid -vf ' + drawtext + ',scale=720:480" -threads 0 ' + mpeg
		...
		try:
			mp4str = 'ffmpeg -i ' + mov + ' -filter_complex "[0:a:0][0:a:1]amerge=inputs=2[a]" -c:v mpeg4 -b:v 372k -pix_fmt yuv420p -r 29.97 -vf ' + drawtext + ',scale=420:270" -c:a aac -ar 44100 -map 0:v -map "[a]" -map -0:d -threads 0 ' + mp4
